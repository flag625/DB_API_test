...
应用SQLObject ORM 搭配后端数据库 MySQL 和 SQLite
非原创代码
"""
...
DSNs = {
    'mysql' : 'mysql://root:a12345678@localhost:3306/%s' %DBNAME,
    'sqlite': 'sqlite:///:memory:',
}

class Users(SQLObject):
    login  = StringCol(length=NAMWLEN)
    userid = IntCol()
    projid = IntCol()
    def __str__(self):
...
def main():
    printf('*** Connect to %r databases' %DBNAME)
    time.sleep(0.01)
    db = setup()
    if db not in DSNs:
